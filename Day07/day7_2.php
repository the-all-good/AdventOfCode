...
function sort_input(Array $input){
    ...
    foreach($input as $game){
        ...
        // Count all J cards and add that to the next highest count.
        $cards = sort_joker($cards);
...
function sort_joker($input){
    if(array_key_exists('J',$input) && $input['J'] === 5){
        return $input;
    }
    if(array_key_exists('J', $input)){
        $joker_count = $input['J'];
        unset($input['J']);
        arsort($input);
        $input[key($input)] += $joker_count;
    }
    return $input;
}
